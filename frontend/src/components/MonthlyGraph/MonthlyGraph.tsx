...
export default function MonthlyGraph({ type }: MonthlyGraphProps) {
    ...
    const chartData: DataPoint[] = Object.values(monthlyData)
    ...
    return (
        <Card className="min-w-0 w-full">
            <CardHeader className="pb-2">
                <div className="flex justify-between items-center">
                    <CardTitle className="text-base font-medium">{config.title}</CardTitle>
                    <span className={`text-sm font-medium text-${type === 'income' ? 'success' : 'destructive'}`}>
            ...
            <CardContent className="p-0">
                <div className="w-full h-[180px] flex items-center justify-center">
                    <div className="w-full h-[160px] px-6 py-4">
                        <ChartContainer config={{ [type]: config }} className="w-full h-full">
                                ...
                                >
                                    ...
                                    <ChartTooltip
                                        content={({ active, payload }) => {
                                            ...
                                            const value = type === 'spending' ? -data.value : data.value
                                            return (
                                                <div className="rounded-lg border bg-background p-2 shadow-sm">
                                                    ...
                                                    <div className={`text-sm font-bold text-${type === 'income' ? 'success' : 'destructive'}`}>
