...
export default function Networth({ data }: NetworthProps) {
  ...
  const canSlideRight = startIndex + getRangeMonths(selectedRange) < data.length;
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent className="h-[300px] flex items-center justify-center text-muted-foreground">
            No transactions available. Add some transactions to see your net worth over time.
          </CardContent>
        </Card>
      </div>
    )
  ...
  const getPeriodDateRange = (periodData: NetWorthDataPoint[]) => {
    ...
    return `${startDate.toLocaleString('default', { month: 'long' })} - ${endDate.toLocaleString('default', { month: 'long' })} ${endDate.getFullYear()}`;
  };
                      ...
                      return (
                        <div className="rounded-lg border bg-background p-2 shadow-sm">
                          <div className="text-sm text-muted-foreground">
                            {data.date.toLocaleString('default', { month: 'long', year: 'numeric', day: 'numeric' })}
                          </div>
